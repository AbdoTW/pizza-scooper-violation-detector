...
class StateBasedHygieneViolationDetector:
    def __init__(self):
        ...
        self.stabilization_period = 0  # 2 second stabilization period
        ...
        # ROI-1 transition delay mechanism
        self.roi1_disappear_time = None
        self.roi1_to_roi2_delay = 1.0
        self.last_roi1_state_time = None
        
        # ROI-2 state transition delay mechanism
        self.roi2_scooper_disappear_time = None
        self.roi2_with_to_without_delay = 1.0
        self.last_roi2_with_scooper_time = None
        
    def _determine_current_state(self, roi1_hands, roi2_hands, roi1_scoopers, roi2_scoopers):
        ...
        if roi1_hand_count > 0:
            self.roi1_disappear_time = None
            self.last_roi1_state_time = current_time
            self.roi2_scooper_disappear_time = None
            self.last_roi2_with_scooper_time = None
            return ROIState.ROI_1
        else:
            if self.last_roi1_state_time is not None:
                if self.roi1_disappear_time is None:
                    self.roi1_disappear_time = current_time
                
                elapsed_since_disappear = current_time - self.roi1_disappear_time
        ...
        if roi2_hand_count > 0:
            ...
            if roi2_scooper_count > 0:
                self.roi2_scooper_disappear_time = None
                self.last_roi2_with_scooper_time = current_time
                return ROIState.ROI_2_WITH_SCOOPER
            else:
                if self.last_roi2_with_scooper_time is not None:
                    if self.roi2_scooper_disappear_time is None:
                        self.roi2_scooper_disappear_time = current_time
                    
                    elapsed_since_scooper_disappear = current_time - self.roi2_scooper_disappear_time
    ...
    def _update_state_history(self, new_state):
    ...
    def check_violation(self, roi1_hands, roi2_hands, roi1_scoopers, roi2_scoopers):
        """
        Enhanced violation detection with modified state machine logic
        Returns: (violation_detected, roi1_hand_count, roi2_hand_count, roi1_scooper_count, 
                roi2_scooper_count, stabilization_remaining, display_state, violation_reason, new_violation)
        """
        current_time = time.time()
        ...
        # Determine current state with new logic
        new_state = self._determine_current_state(roi1_hands, roi2_hands, roi1_scoopers, roi2_scoopers)
        state_changed = self._update_state_history(new_state)
        ...
        if potential_violation:
            if self.potential_violation_start is None:
                self.potential_violation_start = current_time
                print(f"🟡 POTENTIAL VIOLATION STARTED - stabilization timer begins")
                violation_detected = False
            else:
                stabilization_time = current_time - self.potential_violation_start
        ...
        if violation_detected and not self.violation_active:
            ...
            violation_info = {
                'timestamp': float(current_time),  # Ensure it's Python float
                ...
                'state_history': [str(state.value) for state in list(self.state_history)[-3:]],
            ...
            new_violation = {
                'timestamp': float(current_time),  # Python float, not numpy
            ...
            # Violation resolved
            self.violation_active = False
            duration = current_time - self.violation_start_time if self.violation_start_time else 0
            print(f"Violation resolved after {duration:.1f} seconds")

        # Calculate remaining stabilization time for display
        stabilization_remaining = 0
        if self.potential_violation_start is not None and not violation_detected:
            elapsed = current_time - self.potential_violation_start
    ...
    def reset_for_new_video(self):
        """Reset all violation detection state for a new video"""
...
class HygieneDetectionService:
    def __init__(self):
        ...
        self.roi1_hand_colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (255, 0, 255)]
        self.roi1_scooper_colors = [(0, 165, 255), (0, 255, 255), (255, 100, 100), (100, 255, 100), (100, 100, 255)]
        self.roi2_hand_colors = [(128, 0, 0), (0, 128, 0), (0, 0, 128), (128, 128, 0), (128, 0, 128)]
        self.roi2_scooper_colors = [(0, 80, 128), (0, 128, 128), (128, 50, 50), (50, 128, 50), (50, 50, 128)]
    ...
    def load_yolo_model(self):
        ...
        try:
            # Update this path to your scooper detection model
            model_path = "/home/tawfik/git/pizza_violation_microservices/models/best_scooper_pizzaStoreLarge_largeModel.pt"
            self.model = YOLO(model_path)
            print(f"YOLO hygiene model loaded successfully")
        except Exception as e:
            print(f"Error loading YOLO model: {e}")
            # Fallback to a standard model if custom model not available
    ...
    def process_frame_message(self, channel, method, properties, body):
        ...
        try:
            ...
            new_violation) = self.violation_detector.check_violation(
                roi1_tracked_hands, roi1_tracked_scoopers, roi2_tracked_hands, roi2_tracked_scoopers
            )
    ...
    def _analyze_extended_pattern(self):
        """
        Analyze extended state history patterns for complex violation scenarios
        Used as fallback when basic 3-state pattern is inconclusive
        """
    ...
    def process_roi_config_selection(self, video_name=None, custom_roi=None):
        """
        Process ROI configuration based on video name or custom configuration
        This allows dynamic ROI setup for different videos
        """
        if custom_roi:
            # Use custom ROI configuration
            if 'roi1' in custom_roi and 'roi2' in custom_roi:
                self.roi1_points = np.array(custom_roi['roi1'], dtype=np.int32)
                self.roi2_points = np.array(custom_roi['roi2'], dtype=np.int32)
                print(f"Using custom ROI configuration: {custom_roi.get('label', 'Custom')}")
                return True
        
        if video_name:
            # Map video name to predefined configuration
            config_mapping = {
                'test_video1.mp4': 'video_1',
                'test_video2.mp4': 'video_2', 
                'test_video3.mp4': 'video_3'
            }
            
            config_key = config_mapping.get(video_name, 'default')
            if config_key in ROI_CONFIGURATIONS:
                roi_config = ROI_CONFIGURATIONS[config_key]
                self.roi1_points = np.array(roi_config['roi1'], dtype=np.int32)
                self.roi2_points = np.array(roi_config['roi2'], dtype=np.int32)
                print(f"Using {roi_config['label']} for video: {video_name}")
                return True
        
        # Default fallback
        default_config = ROI_CONFIGURATIONS['default']
        self.roi1_points = np.array(default_config['roi1'], dtype=np.int32)
        self.roi2_points = np.array(default_config['roi2'], dtype=np.int32)
        print("Using default ROI configuration")
        return True

    def save_violation_states_for_hands(self, roi1_hands, roi2_hands):
        """
        Save the 3 states for each detected hand in ROI-1 and ROI-2
        This method tracks individual hand states throughout the detection process
        """
        current_time = time.time()
        hand_states = {}
        
        # Process ROI-1 hands
        for hand_id, hand_data in roi1_hands.items():
            if hand_data['active']:
                # Determine hand's current state
                current_state = ROIState.ROI_1
                
                # Initialize hand state tracking if not exists
                if not hasattr(self, 'hand_state_history'):
                    self.hand_state_history = {}
                    
                if hand_id not in self.hand_state_history:
                    self.hand_state_history[hand_id] = {
                        'states': deque(maxlen=10),  # Keep last 10 states
                ...
                # Update hand state
                hand_history = self.hand_state_history[hand_id]
                if not hand_history['states'] or hand_history['states'][-1] != current_state:
                    hand_history['states'].append(current_state)
                    hand_history['timestamps'].append(current_time)
                    hand_history['last_update'] = current_time
                    hand_history['roi'] = 'ROI1'
                    
                hand_states[f"roi1_hand_{hand_id}"] = {
                    'current_state': current_state.value,
                    'state_history': [state.value for state in hand_history['states']],
                    'timestamps': list(hand_history['timestamps']),
                    'roi': 'ROI1',
                    'centroid': hand_data['centroid'],
                    'active_duration': current_time - hand_history['first_seen']
                }
        
        # Process ROI-2 hands
        for hand_id, hand_data in roi2_hands.items():
            if hand_data['active']:
                # Determine hand's current state based on scooper presence
                # This requires checking if there are scoopers in ROI-2
                roi2_has_scoopers = any(scooper['active'] for scooper in self.roi2_scooper_tracker.objects.values())
                current_state = ROIState.ROI_2_WITH_SCOOPER if roi2_has_scoopers else ROIState.ROI_2_WITHOUT_SCOOPER
                ...
                if not hand_history['states'] or hand_history['states'][-1] != current_state:
                    ...
                    hand_history['timestamps'].append(current_time)
                    hand_history['last_update'] = current_time
                    hand_history['roi'] = 'ROI2'
                    
                hand_states[f"roi2_hand_{hand_id}"] = {
                    'current_state': current_state.value,
                    'state_history': [state.value for state in hand_history['states']],
                    'timestamps': list(hand_history['timestamps']),
                    'roi': 'ROI2',
        ...
        # Clean up inactive hands from history (optional)
        if hasattr(self, 'hand_state_history'):
            # Remove hands that haven't been seen for more than 30 seconds
            inactive_threshold = 30.0
            hands_to_remove = []
            for hand_id, history in self.hand_state_history.items():
                if current_time - history['last_update'] > inactive_threshold:
                    hands_to_remove.append(hand_id)
            
            for hand_id in hands_to_remove:
                del self.hand_state_history[hand_id]
    ...
    def debug_roi_visualization(self, frame):
        """Save a frame with ROI visualization for debugging"""
        import cv2
        import uuid
        ...
        cv2.polylines(debug_frame, [self.roi1_points], True, (0, 255, 255), 3)  # Yellow ROI-1
        cv2.polylines(debug_frame, [self.roi2_points], True, (255, 0, 255), 3)  # Magenta ROI-2
        
        # Add ROI labels
        cv2.putText(debug_frame, "ROI-1", tuple(self.roi1_points[0]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
        cv2.putText(debug_frame, "ROI-2", tuple(self.roi2_points[0]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)
        
        # Save debug frame
        debug_filename = f"debug_roi_{uuid.uuid4().hex[:8]}.jpg"
        cv2.imwrite(debug_filename, debug_frame)
        print(f"DEBUG: ROI visualization saved as {debug_filename}")
        
        return debug_filename
    
    
def main():
    """Main function to run the detection service"""
